...
function findGids(){
  ...
  fetch(base+"/htmlview",{cache:"no-store",credentials:"omit"}).then(function(r){ return r.text(); }).then(function(t){
    ...
    var want={settings:"fieldyouranswer", teams:"teamnamepool", schedule:"datefaceoffawayteam", stats:"noplayergpgapts", rinks:"rinknameaddress", sponsors:"sponsortier"},
    ...
    if(out.settings||out.teams||out.schedule){
      state.gidHint = '  gids: { settings: "'+(out.settings||"?")+'", teams: "'+(out.teams||"?")+
                '", schedule: "'+(out.schedule||"?")+'"'+(out.stats?', stats: "'+out.stats+'"':"")+(out.rinks?', rinks: "'+out.rinks+'"':"")+(out.sponsors?', sponsors: "'+out.sponsors+'"':"")+' },\n  (paste that line over the gids line in index.html)';
    } else {
