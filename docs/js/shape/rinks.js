...
var SPEC_RINKS = {
  name: ["rinkname", "rink", "name", "arena", "venue"],
  address: ["address", "streetaddress", "location"],
  notes: ["notes", "note"]
};

/**
 * Reads the Rinks tab into a prototype-less map, norm(rink name) -> row.
 *
 ...
 *
 * @param {string[][]} rows
 * @returns {Object} norm(name) -> {name, address, notes}
 */
function shapeRinks(rows) {
  ...
  if (!h || h.map.name === undefined) {
    log("rinks: no 'Rink name' header found, tab ignored");
    return out;
  }
